  ...
  return (
    <header className="w-full flex justify-center pt-8 sticky top-0 z-50">
      <nav className="backdrop-blur-[7.5px] bg-[#171717] border border-white rounded-[50px] w-full max-w-[1298px] mx-4 shadow-lg relative">
        {/* Desktop Navigation */}
        <div className="hidden lg:flex items-center justify-between px-[10px] h-[86px]">
          <NavLink text="Home" targetId="home" isActive={activeSection === 'home'} />
          <NavLink text="About" targetId="about" isActive={activeSection === 'about'} />
          <NavLink text="Service" targetId="services" isActive={activeSection === 'services'} />
          
          <div className="flex gap-[9px] items-center px-[84px] py-[19px] rounded-[60px]">
            <div className="bg-gradient-to-r from-[#fd853a] to-[#fd853a] p-[2px] rounded-[50px] size-[48px]">
          ...
          <a 
            className="px-[40px] py-[20px] rounded-[60px] hover:bg-white/5 transition-colors" 
          ...
          <NavLink text="Project" targetId="projects" isActive={activeSection === 'projects'} />
          <NavLink text="Contact" targetId="contact" isActive={activeSection === 'contact'} />
        </div>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 right-0 mt-2 bg-[#171717] border border-white rounded-[24px] p-4 flex flex-col gap-2 shadow-xl">
            ...
            <a 
              className="px-6 py-3 rounded-[60px] text-white font-['Poppins'] text-lg hover:bg-white/10 transition-colors" 
              href="https://drive.google.com/file/d/1v2jUoGQZCjjWCCprGsCjLl1i7r4mo1In/view?usp=sharing"
              target="_blank"
              rel="noopener noreferrer"
...
function NavLink({ text, targetId, isActive, isMobileHeader }: { text: string; targetId?: string; isActive?: boolean; isMobileHeader?: boolean }) {
  ...
  return (
    <div 
      ...
      } ${isMobileHeader ? "px-6 py-2 rounded-[60px]" : "px-[40px] py-[20px] rounded-[60px]"}`}
    >
